...
document.addEventListener('click',async e=>{const btn=e.target.closest('[data-assign]');if(!btn)return;const c=btn.closest('.card');const name=c.querySelector('.sub').textContent.split(' · ')[0];const role=c.querySelector('[data-role]').value;const f=c.querySelector('[data-role-feedback]');if(!role){f.textContent='Choose a role first.';return}f.textContent='Assigning…';try{await get('/api/roles/assign',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({role,account:name})});f.textContent=`${role} assigned; refreshing…`;await refresh()}catch(err){f.textContent=err.message}});$('#refresh').addEventListener('click',()=>refresh().catch(e=>$('#accounts').innerHTML=`<div class="empty">${esc(e.message)}</div>`));refresh().catch(e=>{$('#health').textContent='Unavailable';$('#accounts').innerHTML=`<div class="empty">${esc(e.message)}</div>`});"""
